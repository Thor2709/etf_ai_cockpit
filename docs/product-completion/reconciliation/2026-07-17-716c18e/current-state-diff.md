...
Baseline: `716c18e578903d000de9a8dbc441f43f014a95d4`. This inventory is evidence for sequencing, not a claim that product features were implemented by this task.
...
`src/etf_cockpit/app/router.py` currently registers **28** routes: `/`, `/portfolio`, `/signals`, `/screener`, `/risk`, `/etf`, `/backtests`, `/chatgpt`, `/providers`, `/evidence`, `/filings`, `/etf-disclosures`, `/news-context`, `/data-models`, `/settings`, `/diagnostics`, `/errors`, `/data-health`, `/universe`, `/onboarding`, `/what-changed`, `/instrument`, `/import-export`, `/system-map`, `/help`, `/decision-journal`, `/jobs`, `/release-readiness`. The count is higher than the plan's earlier estimate; this reconciliation records the source-of-truth count without altering routing.
