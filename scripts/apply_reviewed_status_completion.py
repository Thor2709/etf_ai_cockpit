...
try:
    ...
    from scripts.issue_registry_core import REGISTRY_PATH
except ModuleNotFoundError:
    import sync_github_issues as sync
    from issue_registry_core import REGISTRY_PATH
    ...
    *,
    ...
    if main_ref is None:
        completed = subprocess.run(
            ["git", "merge-base", "--is-ancestor", expected_parent, expected_head],
            cwd=root,
            check=False,
        )
...
def validate_candidate(
    ...
    if not from_status or from_status == "integrated":
        raise ValueError("candidate from status is invalid")
