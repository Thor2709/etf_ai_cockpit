# Task 1 review package - no-Git base snapshot to current workspace

## Review basis
- Base: immutable Task 1 snapshot captured before the fresh implementer was dispatched; no Git repository or commit exists.
- Head: current workspace after the implementer reported DONE_WITH_CONCERNS.
- Scope: Wave 0 Task 1 only. Ignore generated __pycache__ files.
- Limitation: tests/test_closure_matrix.py was added to the task scope after repository inspection exposed its exact-41 regression. Its base content below is reconstructed from the pre-dispatch read and normalises line endings; the pre-task SHA-256 manifest is retained at .ai_worklog/task-1-code-manifest-before.csv.

## Changed task paths
- src/etf_cockpit/core/closure.py
- src/etf_cockpit/operations/__init__.py (new)
- src/etf_cockpit/operations/models.py (new)
- configs/closure_matrix.yaml
- tests/test_closure_matrix.py
- tests/operations/test_verification_records.py (new)
- tests/release/test_issue_evidence.py (new)
...
## Checkpoint and review inputs
- Implementer report: .ai_worklog/task-1-report.md
- Plan brief: .ai_worklog/task-1-brief.md
- Current ledger: docs/superpowers/plans/2026-07-11-etf-ai-cockpit-progress-ledger.md
- Current run checkpoint: RUN_STATE.json
