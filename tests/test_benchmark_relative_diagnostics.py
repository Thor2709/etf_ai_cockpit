...
def test_currency_mismatch_requires_explicit_fx_evidence() -> None:
    returns = [
        _return("portfolio", "P1", 0.04),
        _return("benchmark", "B1", 0.03, currency="USD"),
    ]
    without_fx = [row for row in _complete() if row.dimension != "currency_fx"]
    with pytest.raises(
        BenchmarkDiagnosticsError, match="currency mismatch requires explicit"
    ):
        _build(without_fx, returns)
    assert _build(_complete(), returns).dimensions[4].status == "available"
...
def test_cutoff_and_effective_period_precede_known_revision_order() -> None:
    ...
    new_early = _evidence("factor", "Momentum", 0.002, revision=1)
    future = replace(
        new_early,
        contribution=0.8,
