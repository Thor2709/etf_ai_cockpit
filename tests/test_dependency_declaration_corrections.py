...
def test_exact_source_dependencies_and_lanes():
...
@pytest.mark.parametrize("tail", ["downstream `ISSUE-0152`", "notes `ISSUE-0152`", "execution false"])
def test_compact_clause_boundary(tail):
    assert core._compact_metadata(f"**Canonical metadata:** P0; depends on `ISSUE-0074`; {tail}.")["Blocking dependencies"] == "`ISSUE-0074`"
...
def test_writer_rejects_invalid_batch_without_mutation(mutation):
    control = core.load_control_state(ROOT)
