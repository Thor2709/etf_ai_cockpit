...
tests pass. The next action is a bounded evidence-only transaction changing
only those two ISSUE-0089 edges from `unresolved` to `complete`, followed by
deterministic generation and protected validation. No product status,
provider state, broker authority or `execution_allowed=false` changes in the
