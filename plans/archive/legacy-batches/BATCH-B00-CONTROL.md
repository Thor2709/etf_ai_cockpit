...
This file preserves the complete pre-alignment `plans/BATCH-B00-CONTROL.md` record from PR #632 head `e30ffd6237c615cb9e68cdd362289fa2785a7a80`. It is historical evidence, not current operational instruction. Current mechanics are in `docs/product-completion/DELIVERY_WORKFLOW.md`.
...
- `REVISE` the first ISSUE-0084 readiness proposal proved that schema 1.2 coupled dependency evidence to a status transition: it could label the issue `ready` while the separate ISSUE-0073 edge remained unresolved. Checkpoint `0b401725a3c29dfd01536cdf16dfd2ce321c0797` was rejected by independent review, remains unpushed and is not release evidence.
- `VERIFIED` the bounded B00 repair adds schema 1.3 `generation_base_and_dependency_edge_evidence`: exactly one declared edge may advance through a distinct reviewed control event while programme status, status-transition metadata and acceptance evidence remain byte-equivalent. The guard rejects other record/top-level drift and now enforces `programme_status=ready` only when the deterministic implementation-readiness projection is true.
- `VERIFIED` the first edge-only use records only ISSUE-0084→ISSUE-0082 as `complete` against merged product commit `d590c8fef1f645614e7ed326e60b31d823d54113`; ISSUE-0084 remains `planned`, ISSUE-0073 remains `unresolved`, and `execution_allowed=false`. The stale control generation base is refreshed from `72b69b356a70a68a855513df82d107fec83e2c85` to current `origin/main` `ad783e517be68882934300df73106891ae6e3c05`.
- `IN_PROGRESS` this protected programme-control/status-tooling repair requires focused control/registry/status/supply-chain checks, independent review and an immediate complete Linux plus Windows packaged release gate under the product owner's validation-cadence amendment.
- `REVISE` independent review reproduced three audit-authority bypasses invisible to generated-registry comparison: unallowlisted control metadata/top-level fields and a second no-op edge event. The repair now compares all non-record control fields to authoritative origin with only the exact generation-base metadata refresh allowed, binds schema 1.3 replay to its declared issue/dependency, and rejects unchanged edge evidence or verified commits.
- `VERIFIED` final independent re-review passed with no material finding. Adversarial regressions reject injected top-level `execution_allowed=true`, metadata drift and a second no-op event; the exact ISSUE-0084→ISSUE-0082 event accepts. Final local evidence passes 93 focused/control/UI tests, 55 affected integration tests, the real guard, registry/status/completion freshness, Ruff, scoped MyPy, compileall, changed source smoke, supply-chain and diff hygiene.
