...
from etf_cockpit.chatgpt_bridge.audit_packet import _contains_unredacted_secret, validate_audit_archive
...
def import_audit_json(path: Path, config: AppConfig) -> ChatGPTAudit | ChatGPTAuditV2:
    ...
    if path.suffix.lower() == ".zip":
        ...
        if not report.valid:
            raise AuditImportError(f"Audit archive rejected: missing={report.missing}, checksums={report.checksum_errors}, secrets={report.secret_findings}")
        raise AuditImportError("Audit packet archives are evidence exports; import a schema-validated commentary JSON note instead.")
    try:
        raw_text = path.read_text(encoding="utf-8")
    except (OSError, UnicodeError) as exc:
        raise AuditImportError(f"Audit JSON could not be read: {type(exc).__name__}") from exc
