...
_GUIDANCE_SOURCE_AUTHORITIES = frozenset(
    {"company", "company official", "company_official", "official", "issuer"}
)
_REVIEWED_GUIDANCE_STATUSES = frozenset(
    {"approved", "human_reviewed", "reviewed", "structured", "verified"}
)
...
def profitability_analysis(
    ...
    *,
    ...
    metrics["operating_margin"] = _ratio_metric(
        "operating_margin",
        operating_income,
        revenue,
    ...
    metrics["net_margin"] = _ratio_metric(
        "net_margin", net_income, revenue, "net_income / revenue", frame, "revenue"
    )
    metrics["roa"] = _ratio_metric(
        "roa", net_income, assets, "net_income / assets", frame, "assets"
    )
    metrics["roe"] = _ratio_metric(
        "roe", net_income, equity, "net_income / equity", frame, "equity"
    )
    invested_capital = _sum_if_present(
        equity, debt, -cash if cash is not None else None
    )
    roic = (
        None
        if sector.casefold() in _SPECIAL_SECTORS
        else (
            None
            if operating_income is None or tax is None
            else operating_income * (1.0 - tax)
        )
    )
    metrics["roic"] = _ratio_metric(
        ...
        limitation="Special-sector adapter required."
        if sector.casefold() in _SPECIAL_SECTORS
        else "",
    )
    ...
    metrics["accrual_ratio"] = _ratio_metric(
        ...
        accrual_numerator,
        assets,
        "(net_income - cash_from_operations) / assets",
        frame,
        "assets",
    )
    exceptional = latest.get("exceptional_items")
    metrics["exceptional_item_dependence"] = _ratio_metric(
        "exceptional_item_dependence",
        exceptional,
        net_income,
        "exceptional_items / net_income",
        frame,
        "exceptional_items",
        zero_denominator_status="not_applicable",
    )
    ...
    metrics["margin_stability"] = _metric(
        ...
        limitation="At least two comparable periods are required."
        if len(margins) < 2
        else "",
    )
    ...
    return {
        ...
        "history": {
            ...
            for name, values in {
                ...
                "operating_margin": _derived_history(
                    histories, "operating_income", "revenue"
                ),
...
def balance_sheet_analysis(
    ...
    *,
    ...
    metrics["net_debt"] = _metric(
        "net_debt",
        None if debt is None or cash is None else debt - cash,
        "debt - cash",
        frame,
    )
    metrics["debt_to_equity"] = _ratio_metric(
        "debt_to_equity", debt, equity, "debt / equity", frame, "debt"
    )
    ...
    metrics["quick_ratio"] = _ratio_metric(
        "quick_ratio",
        quick_assets,
        current_liabilities,
...
def valuation_analysis(
    ...
    *,
    ...
    if (
        ...
        and values.get("debt") is not None
        and values.get("cash") is not None
    ):
        values["net_debt"] = values["debt"] - values["cash"]
    relative_metrics = {
        "ev_to_sales": _ratio_metric(
            "ev_to_sales",
            values.get("enterprise_value"),
            values.get("revenue"),
            "enterprise_value / revenue",
            frame,
            "enterprise_value",
        ),
        "ev_to_ebitda": _ratio_metric(
            "ev_to_ebitda",
            values.get("enterprise_value"),
        ...
        "price_to_earnings": _ratio_metric(
            "price_to_earnings",
...
def build_stock_research_report(
    ...
    *,
    ...
    market_inputs: Mapping[str, object] | None = None,
    assumptions: Mapping[str, object] | None = None,
    expectation_evidence: Iterable[object]
    | Mapping[str, object]
    | pd.DataFrame
    | None = None,
    guidance_evidence: Iterable[object]
    | Mapping[str, object]
    | pd.DataFrame
    | None = None,
    as_known_at: str | date | None = None,
    ...
    return {
        ...
        "balance_sheet": balance_sheet_analysis(
            frame, instrument_id=instrument_id, sector=sector, as_known_at=as_known_at
        ),
        ...
        "growth": growth_analysis(
            frame, instrument_id=instrument_id, as_known_at=as_known_at
        ),
        "expectations": _expectations_report(
            frame,
...
def load_optional_research_import(
    path: object, *, instrument_id: str | None = None
) -> pd.DataFrame:
    """Load a local optional evidence file without granting it data authority.

    Shared imports must identify their instrument explicitly so a page cannot
    display another instrument's consensus or guidance. The import file's
    checksum is used when a row does not carry a more specific source checksum;
    source identity, authority, timestamps and licensing are still validated by
    the report builder.
    """
    ...
    try:
        ...
        if suffix == ".csv":
            ...
            return _empty_optional_import(
                candidate, "rejected", f"unsupported_file_type:{suffix or 'none'}"
            )
        checksum_after = _file_sha256(candidate)
    except (ImportError, OSError, UnicodeError, ValueError) as exc:
        return _empty_optional_import(
            candidate, "rejected", f"unreadable_import:{type(exc).__name__}"
        )
    ...
    if instrument_id:
        if "instrument_id" not in frame.columns:
            return _empty_optional_import(
                candidate, "rejected", "missing_instrument_id"
            )
    ...
    if "source_checksum" not in frame.columns:
        ...
        frame["source_checksum"] = frame["source_checksum"].where(
            frame["source_checksum"].notna()
            & frame["source_checksum"].astype(str).str.strip().ne(""),
            checksum,
        )
    frame.attrs.update(
        {
            "import_path": str(candidate),
...
def _empty_optional_import(path: object, status: str, reason: str) -> pd.DataFrame:
    frame = pd.DataFrame()
    frame.attrs.update(
        {"import_path": str(path), "import_status": status, "import_reason": reason}
    )
    return frame
...
def _histories(frame: pd.DataFrame) -> dict[str, list[float]]:
    ...
    for column in ("period_end", "fiscal_year", "period_key"):
        if column not in ordered:
            ordered[column] = ""
    ordered = ordered.sort_values(
        ["period_end", "fiscal_year", "period_key"], kind="stable", na_position="last"
    )
    result: dict[str, list[float]] = {}
    for metric, rows in ordered.groupby("canonical_metric", dropna=True):
        values = (
            pd.to_numeric(rows["value"], errors="coerce")
            .dropna()
            .astype(float)
            .tolist()
        )
...
def _period_rows(frame: pd.DataFrame) -> list[dict[str, object]]:
    ...
    work = work.sort_values(
        ["__sort_end", "__period_end", "period_key", "canonical_metric", "source_id"],
        kind="stable",
        na_position="last",
    )
    periods: list[dict[str, object]] = []
    grouped = work.groupby(
        ["period_type", "period_key", "__period_end"], sort=False, dropna=False
    )
    for (period_type, period_key, period_end), rows in grouped:
        ...
        for metric, metric_rows in rows.groupby(
            "canonical_metric", dropna=True, sort=False
        ):
            ordered = metric_rows.assign(
                __restatement_rank=metric_rows["restatement_kind"].map(
                    _restatement_rank
                )
            ).sort_values(
                ["filed", "__restatement_rank", "accession", "source_id"],
                kind="stable",
                na_position="last",
            )
            numeric = ordered.assign(
                __numeric=pd.to_numeric(ordered["value"], errors="coerce")
            ).dropna(subset=["__numeric"])
            if numeric.empty:
                continue
            raw_values[str(metric)] = float(numeric.iloc[-1]["__numeric"])
            raw_sources[str(metric)] = tuple(
                sorted(
                    {
                        str(value)
        ...
        for logical, aliases in _GROWTH_ALIASES.items():
            for alias in aliases:
                if alias in raw_values:
                    ...
                    break
        if (
            "earnings_per_share" not in values
            and {"net_income", "shares_outstanding"} <= values.keys()
            and values["shares_outstanding"] != 0
        ):
            values["earnings_per_share"] = (
                values["net_income"] / values["shares_outstanding"]
            )
            sources["earnings_per_share"] = tuple(
                sorted(
                    set(sources.get("net_income", ()))
                    | set(sources.get("shares_outstanding", ()))
                )
            )
...
def _growth_series(
    ...
    *,
    ...
    for period in periods:
        ...
        value = next(
            (
                _float(values.get(alias))
                for alias in aliases
                if _float(values.get(alias)) is not None
            ),
            None,
        )
        formula = next(
            (
                str(period.get("formulas", {}).get(alias, ""))
                for alias in aliases
                if alias in period.get("formulas", {})
            ),
            "",
        )
        source_ids = next(
            (
                tuple(period.get("source_ids", {}).get(alias, ()))
                for alias in aliases
                if alias in period.get("source_ids", {})
            ),
            (),
        )
        if (
            value is None
            and derived_from
            and all(_float(values.get(item)) is not None for item in derived_from)
        ):
            ...
            if numerator is not None and denominator not in (None, 0):
                ...
                source_ids = tuple(
                    sorted(
                        set(period.get("source_ids", {}).get(derived_from[0], ()))
                        | set(period.get("source_ids", {}).get(derived_from[1], ()))
                    )
                )
        history.append(
            {
            ...
            growth.append(
                {
                    ...
                    "status": status,
                    "source_ids": sorted(
                        set(prior.get("source_ids", []))
                        | set(current.get("source_ids", []))
                    ),
                    "execution_allowed": False,
                }
            )
    ...
    return {
        ...
        "status": "available"
        if any(point.get("value") is not None for point in history)
        else "missing",
        "formula": "(current / prior) - 1",
        "execution_allowed": False,
    }
...
def _organic_inorganic_analysis(
    frame: pd.DataFrame, periods: list[dict[str, object]]
) -> dict[str, object]:
    organic = _growth_series(
        periods, "organic_revenue", ("organic_revenue",), basis="aggregate"
    )
    acquired = _growth_series(
        periods, "acquisition_revenue", ("acquisition_revenue",), basis="aggregate"
    )
    flags = [
    ...
    return {
        ...
        "limitation": "Organic growth is unavailable without explicit organic-revenue or acquisition evidence; consolidated growth is not silently labelled organic."
        if not has_organic
        else "",
        "execution_allowed": False,
    }
...
def _expectations_report(
    ...
    *,
    ...
    consensus_rows, consensus_rejected, cutoff = _prepare_optional_rows(
        expectation_evidence,
        instrument_id=instrument_id,
        as_known_at=as_known_at,
        guidance=False,
    )
    guidance_rows, guidance_rejected, _ = _prepare_optional_rows(
        guidance_evidence,
        instrument_id=instrument_id,
        as_known_at=as_known_at,
        guidance=True,
    ...
    return {
        "as_known_at": cutoff,
        "consensus": _consensus_report(
            statements, consensus_rows, consensus_rejected, cutoff
        ),
...
def _consensus_report(
    ...
    if not rows:
        return {
            ...
            "reason": "No licensed point-in-time consensus evidence was supplied.",
            "execution_allowed": False,
        }
    periods = _period_rows(frame)
    ...
    for (metric, period_key), candidates in sorted(grouped.items()):
        ordered = sorted(
            candidates,
            key=lambda item: (str(item["available_at"]), str(item["source_id"])),
        )
        latest_timestamp = ordered[-1]["available_at"]
        latest_rows = [
            item for item in ordered if item["available_at"] == latest_timestamp
        ]
        latest_values = [
            float(item["value"])
        ...
        if actual is not None:
            surprise = {
                ...
                "percent": None if estimate == 0 else actual / estimate - 1.0,
                "formula": "actual - estimate",
                "status": "available",
            }
        revision_value = float(latest["value"]) - float(ordered[0]["value"])
        revision = {
            "value": revision_value,
            "status": "available" if len(ordered) > 1 else "not_available",
            "formula": "latest estimate - earliest estimate",
        }
        dispersion = {
            "value": max(latest_values) - min(latest_values),
            "status": "available" if len(latest_values) > 1 else "not_available",
            "formula": "max(latest-vintage estimates) - min(latest-vintage estimates)",
        }
        ...
        item = {
            ...
            "source_authorities": sorted(
                {str(candidate["source_authority"]) for candidate in candidates}
            ),
            "license_statuses": sorted(
                {
                    str(candidate["license_status"])
            ...
            or {
                ...
                "percent": None,
                "status": "unavailable",
                "reason": "Reported actual for this period is unavailable.",
            },
            "staleness": staleness,
            "revision_history": [
                {key: candidate[key] for key in ("value", "available_at", "source_id")}
                for candidate in ordered
            ],
            "execution_allowed": False,
        }
...
def _guidance_report(
    rows: list[dict[str, object]], rejected: list[str]
) -> dict[str, object]:
    if not rows:
        return {
            "status": "unavailable",
            ...
            "accepted_records": 0,
            "rejected_records": rejected,
            "reason": "Guidance requires structured or human-reviewed evidence with provenance.",
            "execution_allowed": False,
        }
    items = []
    for row in sorted(
        rows, key=lambda item: (str(item["available_at"]), str(item["source_id"]))
    ):
        items.append(
            {
                "status": "available",
...
def _prepare_optional_rows(
    ...
    *,
    ...
    rejected: list[str] = (
        [f"import:{import_reason}"]
        if import_reason and import_reason != "file_not_found"
        else []
    )
    for index, raw in enumerate(frame.to_dict("records")):
        ...
        if instrument_id and row_instrument and row_instrument != str(instrument_id):
            ...
            continue
        reason, row = _validate_optional_row(
            raw, index=index, cutoff=cutoff, guidance=guidance
        )
        if reason:
...
def _validate_optional_row(
    raw: Mapping[str, object], *, index: int, cutoff: str | None, guidance: bool
) -> tuple[str | None, dict[str, object] | None]:
    source_id = _text(raw.get("source_id") or raw.get("source") or raw.get("citation"))
    source_authority = _text(
        raw.get("source_authority") or raw.get("authority") or raw.get("ownership")
    )
    source_kind = _text(raw.get("source_kind") or raw.get("source_type")).casefold()
    provider = _text(
        raw.get("provider") or raw.get("vendor") or raw.get("source_name")
    ).casefold()
    ...
    allowed_authorities = (
        _GUIDANCE_SOURCE_AUTHORITIES if guidance else _CONSENSUS_SOURCE_AUTHORITIES
    )
    source_checksum = _text(raw.get("source_checksum") or raw.get("checksum"))
    available_at = _normalise_timestamp(
        raw.get("available_at") or raw.get("as_of") or raw.get("published_at")
    )
    if (
        source_kind in {"current_analyst", "current_analyst_field", "analyst_current"}
        or "yahoo" in provider
        or "yahoo" in source_id.casefold()
        or ("analyst" in provider and "point" not in source_kind)
    ):
        return f"row_{index}:current_or_restricted_provider_rejected", None
    if (
        not source_id
        or not available_at
        or authority_key not in allowed_authorities
        or not _valid_checksum(source_checksum)
    ):
    ...
    metric = _normalise_optional_metric(
        raw.get("canonical_metric") or raw.get("metric") or raw.get("measure")
    )
    period_key = _text(
        raw.get("period_key") or raw.get("period_end") or raw.get("fiscal_year")
    )
    value = _float(
        raw.get("value")
        if raw.get("value") is not None
        else raw.get("estimate")
    ...
    review_status = (
        _text(raw.get("review_status") or raw.get("review"))
        .casefold()
        .replace(" ", "_")
    )
    ...
    upper = _float(
        raw.get("upper") if raw.get("upper") is not None else raw.get("high")
    )
    ...
    if guidance and (
        review_status not in _REVIEWED_GUIDANCE_STATUSES
        or (value is None and lower is None and upper is None and not text)
    ):
        return f"row_{index}:guidance_not_structured_or_reviewed", None
    if guidance and not metric:
        metric = (
            _normalise_optional_metric(raw.get("guidance_type") or "guidance")
            or "guidance"
        )
    return None, {
        "metric": metric,
        ...
        "license_status": _text(
            ...
            or raw.get("licence_status")
            or _default_license_status(authority_key)
        ),
...
def _actual_for_period(
    periods: list[dict[str, object]], metric: str, period_key: str
) -> float | None:
    aliases = (
        ("operating_profit",)
    ...
    for period in periods:
        if period_key in period.get("period_aliases", []) or period_key == period.get(
            "period_key"
        ):
            values = period.get("values", {})
...
def _evidence_frame(
    evidence: Iterable[object] | Mapping[str, object] | pd.DataFrame,
) -> pd.DataFrame:
...
def _restatement_rank(value: object) -> int:
    return {"reported": 0, "restated": 1, "amended": 2, "corrected": 3}.get(
        _text(value).casefold(), 1
    )
...
def _derived_history(
    histories: Mapping[str, list[float]], numerator: str, denominator: str
) -> list[float]:
    numerators = histories.get(numerator, [])
    denominators = histories.get(denominator, [])
    return [
        float(numerator_value / denominator_value)
        for numerator_value, denominator_value in zip(numerators, denominators)
...
def _ratio_metric(
    ...
    *,
    applicability: str = "applicable",
    limitation: str = "",
    zero_denominator_status: str = "missing",
) -> dict[str, object]:
    ...
    if denominator == 0:
        return _metric(
            ...
            formula,
            frame,
            status_override=zero_denominator_status,
            source_metric=source_metric,
            limitation="Denominator is zero; the ratio is not defined.",
        )
    return _metric(
        name,
...
def _metric(
    ...
    *,
    ...
    status = status_override or (
        "missing" if value is None else "negative" if value < 0 else "available"
    )
    return asdict(
        MetricEvidence(
            name,
            ...
            period,
            source_ids,
            "high" if value is not None and source_ids else "low",
            applicability,
            limitation,
        )
    )
...
def _source_ids(frame: pd.DataFrame, metric: str) -> tuple[str, ...]:
    if (
        frame.empty
        or "canonical_metric" not in frame.columns
...
def _peer_percentile(
    metric: str, value: object, peer_frame: pd.DataFrame | None
) -> float | None:
    ...
    if metric in values:
        ...
        peers = (
            _derived_history(histories, "gross_profit", "revenue")
    ...
    return float(
        100.0
        * (
            sum(item < observed for item in peers)
            + 0.5 * sum(item == observed for item in peers)
        )
        / len(peers)
    )
...
def _quality_components(
    values: Mapping[str, float], histories: Mapping[str, list[float]]
) -> dict[str, object]:
    ...
    for name, value in {
        ...
        "improving_gross_margin": _trend(
            _derived_history(histories, "gross_profit", "revenue"), descending=False
        ),
    }.items():
        components[name] = {
            "value": value
...
def _maturity_timeline(
    values: Mapping[str, float], frame: pd.DataFrame
) -> dict[str, object]:
    ...
    return {
        ...
        "buckets": available,
        "formula": "reported debt maturity buckets",
        "source_ids": sorted(
            {source for name in available for source in _source_ids(frame, name)}
        ),
        "confidence": "high" if available else "low",
        "limitation": "Unavailable maturities are not replaced with zero."
        if not available
        else "",
        "execution_allowed": False,
    }
...
def _distress_metric(
    values: Mapping[str, float], frame: pd.DataFrame, special: bool
) -> dict[str, object]:
    if special:
        return _metric(
            "altman_like_distress",
            None,
            "sector adapter required",
            frame,
            status_override="not_applicable",
            limitation="Altman-like industrial evidence is not applied to banks or insurers.",
        )
    ...
    working_capital = (
        ...
        or values.get("current_liabilities") is None
        else values["current_assets"] - values["current_liabilities"]
    )
    required = (
        working_capital,
        assets,
        values.get("retained_earnings"),
        operating_income,
        equity,
        liabilities,
        revenue,
    ...
    if any(value is None or assets == 0 or liabilities == 0 for value in required):
        return _metric(
            "altman_like_distress",
            None,
            "transparent multi-factor distress evidence",
            frame,
            status_override="missing",
            limitation="All disclosed components are required; result is contextual, not a rating.",
        )
    score = (
        1.2 * working_capital / assets
        + 1.4 * values["retained_earnings"] / assets
        + 3.3 * operating_income / assets
    ...
    return _metric(
        "altman_like_distress",
        score,
        "1.2*WC/assets + 1.4*retained_earnings/assets + 3.3*EBIT/assets + 0.6*equity/liabilities + revenue/assets",
        frame,
        limitation="Contextual distress evidence only; no credit-rating claim.",
    )
...
def _stress_scenarios(
    ...
    for name, revenue_factor, rate_add in (
        ("revenue_down_20", 0.8, 0.0),
        ("refinance_rate_up_200bp", 1.0, 0.02),
    ):
        if revenue is None or margin is None or interest is None or debt is None:
            scenarios[name] = {
                "status": "missing",
                "confidence": "low",
                "assumptions": {"revenue_factor": revenue_factor, "rate_add": rate_add},
                "execution_allowed": False,
            }
            continue
        ...
        scenarios[name] = {
            ...
            else stressed_operating_income / stressed_interest,
            "execution_allowed": False,
        }
    return scenarios
...
def _intrinsic_value(
    values: Mapping[str, float], assumptions: Mapping[str, object]
) -> dict[str, object]:
    ...
    if (
        ...
        return {
            "status": "unavailable",
            "confidence": "low",
            "reason": "free cash flow, share count, net debt, forecast and explicit scenario assumptions are required",
            "scenarios": {},
            "execution_allowed": False,
        }
    ...
    for name, raw in scenarios.items():
        ...
        cash_flow = (
            fcf
            if margin is None
            else fcf * (margin / max(abs(values.get("operating_income", 1.0)), 1e-12))
        )
        present_value = sum(
            cash_flow * (1.0 + growth) ** year / (1.0 + discount) ** year
    ...
    if not results:
        return {
            "status": "unavailable",
            "confidence": "low",
            "reason": "no valid scenario assumptions",
            "scenarios": {},
            "execution_allowed": False,
        }
    return {
...
def _reverse_dcf(
    ...
    def equity_for(growth: float) -> float:
        pv = sum(
            fcf * (1.0 + growth) ** year / (1.0 + discount) ** year
            for year in range(1, years + 1)
        )
        terminal = (
            fcf
    ...
    if not (equity_for(low) <= target <= equity_for(high)):
        return {
            "status": "unavailable",
            "confidence": "low",
            "reason": "market value is outside the bounded growth search",
            "execution_allowed": False,
        }
    for _ in range(80):
        middle = (low + high) / 2.0
...
def _model_disagreement(
    intrinsic: Mapping[str, object], residual: Mapping[str, object]
) -> dict[str, object]:
    values = [
        result.get("per_share")
        for result in (intrinsic.get("scenarios", {}) or {}).values()
        if isinstance(result, Mapping) and result.get("per_share") is not None
    ]
    ...
    return {
        ...
        "confidence": "scenario_only" if values else "low",
        "execution_allowed": False,
    }
