...
def build_benchmark_relative_report(
    *,
    ...
    if portfolio.currency != benchmark.currency and not any(
        row.dimension == "currency_fx" for row in selected
    ):
        raise BenchmarkDiagnosticsError(
            "currency mismatch requires explicit currency_fx contribution evidence"
        )
...
def _chronology(start: str, end: str, effective: str, known: str) -> None:
    if _timestamp(start, "start_at") >= _timestamp(end, "end_at"):
        raise BenchmarkDiagnosticsError("start_at must precede end_at")
    if _timestamp(effective, "effective_at") < _timestamp(start, "start_at"):
        raise BenchmarkDiagnosticsError(
            "effective_at cannot precede the evidence horizon"
        )
    _timestamp(known, "known_at")
